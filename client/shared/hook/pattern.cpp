...
void gtamp::hook::pattern::search()
{
...
void gtamp::hook::pattern::search_avx(__m256i &value, __m256i &mask, uintptr_t data)
{
	// Load the data
...
void gtamp::hook::pattern::search_sse(__m128i &value, __m128i &mask, uintptr_t data)
{
	// Load the data
...
void gtamp::hook::pattern::create_value(std::string pattern)
{
...
void gtamp::hook::pattern::create_mask(std::string pattern)
{
	// For each character in the string
	for (auto &ch = pattern.begin(); ch != pattern.end(); ch++)
	{
...
bool gtamp::hook::pattern::is_avx_supported()
{
...
bool gtamp::hook::pattern::is_sse_supported()
{
	int cpuinfo[4];
	__cpuid(cpuinfo, 1);
